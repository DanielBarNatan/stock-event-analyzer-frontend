...
export default function RegisterForm() {
  ...
  const renderBackgroundElements = () => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      elements.push(
        <div 
          key={i}
          className="absolute rounded-full bg-gradient-to-r from-purple-400 to-indigo-500 opacity-20 animate-pulse"
          style={{
            top: `${top}%`,
            left: `${left}%`,
            width: `${size}px`,
  ...
  return (
    <div className="min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-gray-900 via-purple-900 to-indigo-900 relative">
      {/* Animated background elements */}
      {renderBackgroundElements()}
      
      {/* Glassmorphism glowing circle */}
      <div className="absolute w-[600px] h-[600px] rounded-full bg-gradient-to-r from-purple-500/20 to-blue-500/20 blur-3xl top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 animate-pulse"></div>
      ...
      <div className="absolute w-full h-full overflow-hidden opacity-20 pointer-events-none">
        <div className="absolute top-0 left-0 right-0 bottom-0 animate-[spin_20s_linear_infinite]">
          {Array.from({ length: 10 }).map((_, i) => (
            <div 
              key={i} 
              className="absolute top-1/2 left-1/2 w-full h-[1px] bg-gradient-to-r from-transparent via-purple-400 to-transparent"
      ...
      <div className={`w-full max-w-md relative z-10 transition-all duration-1000 ease-out transform ${animate ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
        ...
        <div className="text-center mb-8">
          <div className="relative inline-block mb-6 group">
            <div className="absolute -inset-1 bg-gradient-to-r from-purple-600 to-blue-600 rounded-full opacity-75 blur group-hover:opacity-100 transition duration-1000 group-hover:duration-200 animate-gradient-shift"></div>
          ...
          <div className="flex justify-center space-x-1">
            ...
            <span className="w-2 h-2 bg-indigo-400 rounded-full animate-ping [animation-delay:0.4s]"></span>
          </div>
        </div>

        {/* Registration form card with glassmorphism effect */}
        <div className={`bg-black/20 backdrop-blur-xl rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/10 transition-all ${shake ? 'animate-shake' : ''}`}>
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                <input
                  id="username"
                  type="text"
                  className="peer w-full h-12 px-4 pt-5 bg-black/20 text-white rounded-xl border border-white/10 outline-none transition-all focus:border-purple-500 placeholder-transparent"
                ...
                <label
                  htmlFor="username"
                  className="absolute left-4 top-4 text-sm text-purple-300 transition-all peer-placeholder-shown:text-base peer-placeholder-shown:top-3.5 peer-placeholder-shown:text-white/50 peer-focus:text-sm peer-focus:top-1.5 peer-focus:text-purple-300"
                >
                  Username
                </label>
            ...
            {/* Register button with liquid animation effect */}
            <div className="mt-8">
              <button
                type="submit"
                disabled={isLoading}
                className="group relative w-full h-12 overflow-hidden rounded-xl bg-gradient-to-r from-purple-500 via-indigo-500 to-blue-500 text-white font-medium shadow-lg transition-all hover:shadow-purple-500/25 disabled:opacity-70 disabled:cursor-not-allowed"
              >
            ...
            <div className="mt-6 text-center">
              <p className="text-blue-200/60 text-sm">
                ...
                <a
                  href="/login"
                  className="relative inline-block text-purple-400 hover:text-purple-300 transition-colors group"
                >
